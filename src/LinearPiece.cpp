...
/*
ModSat LinearPiece::returnRepresentModSat()
{
    ...
    for ( Sign J : { P, N } )
    {
        ...
        {
            Clause zeroClau = { -var->zeroVariable(), var->zeroVariable() };
            representation[J].phi = Formula( Formula(zeroClau), Neg );
        }
        else
        {
            if ( ( indexes[J].at(0) == 0 ) && ( alphas.at(indexes[J].at(0)) != 0 ) )
            {
                ...
                {
                    msAux = binaryModSat( betas.at(0)-1, var->newConstant(betas.at(0)) );
                    representation[J].Phi.push_back( Formula(var->constant(betas.at(0)), Formula(msAux.phi,Neg), Equiv) );
                    representation[J].Phi.insert( representation[J].Phi.end(), msAux.Phi.begin(), msAux.Phi.end() );
                }

                if ( betas.at(0) <= 2 )
                {
                    msAux = binaryModSat( alphas.at(0), var->constant(betas.at(0)) );
                    representation[J].phi.addLukaDisjunction( msAux.phi );
                    representation[J].Phi.insert( representation[J].Phi.end(), msAux.Phi.begin(), msAux.Phi.end() );
                }
                else if ( ilogb(alphas.at(0)) <= ilogb(betas.at(0)-1) )
                {
                    for ( int k = 0; k <= ilogb(alphas.at(0)); k++ )
                    {
                        if ( alphas.at(0) >> k & 1 )
                            representation[J].phi.addLukaDisjunction( Formula(var->constant(betas.at(0))+1+k) );
                    }
                }
                else
                {
                    if ( !var->isThereAuxMultVariable(betas.at(0)) )
                        representation[J].Phi.push_back( Formula( var->auxMultVariable(betas.at(0)) , Formula(Clause(2,var->constant(betas.at(0))+ilogb(alphas.at(0)))) , Equiv ) );

                    representation[J].phi.addLukaDisjunction( var->auxMultVariable(betas.at(0)) );
                }
            }
            ...
            for ( auto j = (indexes[J].at(0) == 0 ? 1 : 0); j < indexes[J].size(); j++ )
            {
                if ( alphas.at(indexes[J].at(j)) != 0 )
                {
                    ...
                    {
                        msAux = binaryModSat( betas.at(indexes[J].at(j))-1, var->newConstant( betas.at(indexes[J].at(j)) ) );
                        representation[J].Phi.push_back( Formula( Formula(var->constant( betas.at(indexes[J].at(j)) ) ), Formula(msAux.phi,Neg), Equiv) );
                        representation[J].Phi.insert( representation[J].Phi.end(), msAux.Phi.begin(), msAux.Phi.end() );
                    }

                    unsigned Min;
                    unsigned Max = alphas.at(indexes[J].at(j));
                    if ( Max < betas.at(indexes[J].at(j)) )
                    {
                        Max = betas.at(indexes[J].at(j));
                        Min = alphas.at(indexes[J].at(j));
                    }
                    else
                        Min = betas.at(indexes[J].at(j));
                    ...
                    Clause minClau;
                    if ( Min == 1 )
                        minClau.push_back( auxVar );
                    else
                    {
                        for ( int k = 0; k <= ilogb(Min); k++ )
                        {
                            if ( Min >> k & 1 )
                                minClau.push_back( auxVar+1+k );
                        }
                    }
                    ...
                    {
                        representation[J].phi.addLukaDisjunction( Formula(minClau) );
                        representation[J].Phi.push_back( Formula(msAux.phi, Formula(indexes[J].at(j)), Equiv) );
                    }
                    else
                    {
                        representation[J].phi.addLukaDisjunction( msAux.phi );
                        representation[J].Phi.push_back( Formula(Formula(minClau), Formula(indexes[J].at(j)), Equiv) );
                    }
    ...
    ModSat representationPiece;
    representationPiece.phi = msAux.phi;
    representationPiece.Phi = representation[P].Phi;
    representationPiece.Phi.insert( representationPiece.Phi.end(), representation[N].Phi.begin(), representation[N].Phi.end() );
    representationPiece.Phi.insert( representationPiece.Phi.end(), msAux.Phi.begin(), msAux.Phi.end() );

    return representationPiece;
}
*/
